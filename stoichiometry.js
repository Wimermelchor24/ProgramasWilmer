 ...
 */
function calculateReactantToReactant(knownReactant, targetReactant, products = []) {
    ...
    // 4. Calculate Products generated by the fully reacted known reactant
    const reactionBaseMoles = realMolesKnown / knownReactant.coef;
    const finalProducts = products.map((p, index) => {
        const molesProduced = reactionBaseMoles * p.coef;
        return {
            ...p,
            index,
            molesProduced,
